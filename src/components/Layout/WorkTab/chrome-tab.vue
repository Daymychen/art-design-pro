  ...
  <div class="worktab">
    <div class="scroll-view" ref="scrollRef">
      <!--      <div
            ...
            >
              <div class=":soy: pointer-events-none absolute left-0 top-0 h-full w-full -z-1" style="border: 1px solid red">
                <ChromeTabBg />
              </div>
            </div>-->
      <ul
        class="tabs tabs-chrome"
        ref="tabsRef"
        :style="{ transform: `translateX(${translateX}px)`, transition: `${transition}` }"
      >
        <li
          class="art-custom-card-chrome"
          v-for="(item, index) in list"
          :key="item.path"
          :ref="item.path"
          :class="{ 'activ-tab': item.path === activeTab }"
          :style="'width: ' + chromeTabWidth(item.title) + 'px !important'"
          :id="`scroll-li-${index}`"
          @click="clickTab(item)"
...
<script setup lang="ts">
  ...
  const chromeTabWidth = (titleCode: string) => {
    const title = formatMenuTitle(titleCode)
    const titleArr = title.split('')
    let chineseCharLength = 0
    titleArr.forEach((e: string) => {
      if (isChineseCharacter(e)) chineseCharLength++
      else chineseCharLength += 0.5
    })
    return 140 + (chineseCharLength - 4) * 14
  }
  // 设置过渡动画
  const setTransition = () => {
  ...
  function isChineseCharacter(str: string) {
    return /^[\u4e00-\u9fa5]+$/.test(str)
  }
